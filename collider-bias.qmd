...
2. **M-structure**: When there is a path between two variables that goes through two colliders. For example:
   ```
   A → C ← B
   ↓       ↓
   D → E ← F
   ```
   The "M" shape forms when conditioning on C or on both C and E can induce associations between variables that would otherwise be independent.
